...
\label{eq:Sqrt}
\frac \
  { \vdash |Sqrt| {(} b {,}\; e {,}\; s {)} \to << [[ D> \sqrt{\mathbb{D} {(} b.|x| {,}\; e {,}\; s {)} } ]], s >> } \
  {[[ L> |Sqrt|, |x| -> 256 ]] \to 16} {.}
\end{phiquation}
%
There is also a \emph{dataization} function \(\mathbb{D}(b, e, s)\)
  that normalizes its first argument and then either returns the data
  attached to the $D$-asset of the normal form or morphs the atom
  into a formation and dataizes it (recursively),
  for example:
%
\begin{phiquation*}
\mathbb{D} \lparen [[ |x| -> 42 ]].|x| \trans 42 {,}\; e {,}\; s \rparen \to 42.
\end{phiquation*}
