...
\(\mathcal{B} \times \mathcal{B} \times \mathcal{S} \to \mathcal{E} \times \mathcal{S}\)
  that deterministically maps formation, universe, and state to an expression and a new state.
\end{definition}
...
An object \textbf{formation}, denoted as $[[ B ]]$, ranged over \(\mathcal{B}\) by \(b_i\), is a binding.
\end{definition}

If $ b = [[ B ]] $, then the predicate \(k \in b\) holds if the key \(k\) is present \(B\).
...
A \textbf{domain} of formation \(b\), denoted as \(\bar{b}\), is a sequence of all attributes of \(b\), excluding assets.
\end{definition}

The domain of the formation in \cref{eq:object-example} is \(\langle \ff{b}, \ff{a}, \ff{foo} \rangle\).
...
An \textbf{application}, denoted as $ e_1 ( \tau -> e_2 ) $ or $ e_1 ( \eta_i -> e_2 ) $,
...
A \textbf{primitive}, denoted by \(p\) and ranging over \(\mathcal{P} \subseteq \mathcal{E}\),
