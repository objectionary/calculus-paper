...
The \emph{contextualization} total function \(\mathbb{C} : \mathcal{E} \times \mathcal{B} \to \mathcal{E}\)
  denoted as \( \ctx{e}{b} \), which removes \(\phiTerminal{\xi}\) from the expression,
...
An normal form is \textbf{absolute},
  denoted by \(a\) and ranging over \(\mathcal{A} \subseteq \mathcal{N}\),
  unless it is
  \begin{inparaenum}[a)]
  \item a dispatch with \(\phiTerminal{\xi}\) subject
  or
  \item an application with a non-absolute argument.
  \end{inparaenum}
\end{definition}
...
The reduction relation is \emph{confluent}:
  ...
  then there exists an expression \(e_3\) such that \(e_1 \strans e_3\) and \(e_2 \strans e_3\).
Confluence guarantees that the normal form of an expression, when it exists, is unique,
  irrespective of the order in which the rules are applied.
We have proved this property mechanically in Lean~4,
  with the proof available in a public GitHub repository\footnote{\url{https://github.com/objectionary/proof}}.
...
\newrule[nmz]{Mnmz} \
\frac \
{ << \nf{,}\; u{,}\; s_1 >> \stepto << p{,}\; s_2 >> } \
{ << e{,}\; u{,}\; s_1 >> \stepto << p{,}\; s_2 >> }  \
\;\text{if}\; e \strans \nf \;\text{and}\; e \not\equiv n

\newrule[\lambda]{Mlambda} \
\frac \
{ << e \bullet t{,}\; u{,}\; s_2 >> \stepto << p{,}\; s_3 >>} \
{ <<  [[  B_1 ,  L>  f , B_2  ]]  \bullet{} t{,}\; u{,}\; s_1 >> \stepto << p{,}\; s_3 >> }  \
\;\text{if}\; f {(} [[ B_1, B_2 ]] {,}\; u{,}\; s_1 {)} \to << e{,}\; s_2 >>

\newrule[\Phi]{Mphi} \
\frac \
{ << e \bullet{} t{,}\; u{,}\; s_1 >> \stepto << p{,}\; s_2 >>} \
{ << Q.\tau \bullet{} t{,}\; u{,}\; s_1 >> \stepto << p{,}\; s_2 >> }  \
...
The notation $<< e, u, s_1 >> \stepto << p, s_2 >>$
...
\frac \
{ } \
{ << [[ B_1, D> \delta, B_2 ]] {,}\; u {,}\; s >> \stepto << \delta {,}\; s >> } \
\quad \
\newrule{norm} \
\frac{ << p {,}\; u {,}\; s_2 >> \stepto << \delta {,}\; s_3 >> } \
{ << e {,}\; u {,}\; s_1 >>  \stepto << \delta {,}\; s_3 >> } \
\;\text{if}\; \mathbb{M} {(} e {,}\; u {,}\; s_1 {)} \to << p {,}\; s_2 >>

\newrule{box} \
\frac \
{ << e {,}\; u {,}\; s_1 >> \stepto << \delta {,}\; s_2 >> } \
{ << [[ B_1, @ -> e, B_2 ]] {,}\; u {,}\; s_1 >>  \stepto << \delta {,}\; s_2 >> } \
...
The notation $<< e, u, s_1 >> \stepto << \delta, s_2 >>$
...
The notation \(\mathbb{D}(e, u)\) is a shortened form of \(\mathbb{D}(e, u, s_0)_{(1)}\),
  which is a \emph{pure} dataization --- inputting an empty state of evaluation and ignoring
  the output state of evaluation.
Here and later, the notation \(x_{(i)}\) denotes the \(i\)-th element of the tuple \(x\) (counting starts from one).
