...
\newenvironment{phinoInference}{%
  ...
  \adjustbox{max width=\linewidth,margin=0em .3em}{%
    ...
    \fi%
    \infrule{\phino@premises}{\phino@conclusion}%
...
The \emph{evaluation} function
  ...
  by calling by value the function attached to $L$
  and shifting the \emph{state} of evaluation,
  while the notation \(\phinoEvaluate{b_1}{e}{s_1}{n_2}{s_2}\)
  means that \(b_1\) evaluates to \(n_2\)
  with a side-effect of shifting \(s_1\) to \(s_2\),
  in the universe \(e\):
\begin{phiquation*}
\infrule{f \lparen [[ B_1, L>f, B_2 ]] {,}\; s_1 \rparen \rightarrow \langle n {,}\; s_2 \rangle}{\phinoEvaluate{[[ B_1, L>f, B_2 ]]}{e}{s_1}{n}{s_2}}
\end{phiquation*}
\end{definition}
...
The inference rules at \cref{fig:morphing} inductively describe
  the algorithm of morphing: rules must be applied in any order
  until they stop matching.
The notation \(\phinoMorph{n_1}{e}{s_1}{n_2}{s_2}\)
  means that \(n_1\) morphs to \(n_2\)
  with a side-effect of shifting \(s_1\) to \(s_2\),
  in the universe \(e\).
...
The inference rules in \cref{fig:dataization} inductively
  describe the algorithm of dataization: rules must be applied in any order
  until they stop matching.
The notation \(\phinoDataize{n}{e}{s_1}{\delta}{s_2}\)
  means that \(n\) dataizes to \(\delta\)
  with a side-effect of shifting \(s_1\) to \(s_2\),
  in the universe \(e\).
...
Two expressions \(n_1\) and \(n_2\) are said to be behaviorally equivalent
  ...
  and any \(e\):
\begin{phiquation*}
\phinoDataize{n_1}{e}{n} \quad\text{and}\quad \phinoDataize{n_2}{e}{n}.
\end{phiquation*}
\end{definition}
